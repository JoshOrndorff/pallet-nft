...
/// Associates an commodity with its ID.
    ...
    add_extra_genesis {
        ...
        build(|config: &GenesisConfig<T, I>| {
            for (who, assets) in config.balances.iter() {
                for asset in assets {
                    match <Module::<T, I> as UniqueAssets::<T::AccountId, CommodityId<T>, <T as Trait<I>>::CommodityInfo>>::mint(who, asset.clone()) {
                        Ok(_) => {}
                        Err(err) => { panic!(err) },
                    }
                }
...
decl_module! {
    pub struct Module<T: Trait<I>, I: Instance = DefaultInstance> for enum Call where origin: T::Origin {
        ...
        pub fn mint(origin, owner_account: T::AccountId, commodity_info: T::CommodityInfo) -> dispatch::DispatchResult {
            ...
            let commodity_id = <Self as UniqueAssets<_, _, _>>::mint(&owner_account, commodity_info)?;
        ...
        pub fn burn(origin, commodity_id: CommodityId<T>) -> dispatch::DispatchResult {
            ...
            <Self as UniqueAssets<_, _, _>>::burn(&commodity_id)?;
        ...
        pub fn transfer(origin, dest_account: T::AccountId, commodity_id: CommodityId<T>) -> dispatch::DispatchResult {
            ...
            <Self as UniqueAssets<_, _, _>>::transfer(&dest_account, &commodity_id)?;
...
impl<T: Trait<I>, I: Instance>
    UniqueAssets<T::AccountId, CommodityId<T>, <T as Trait<I>>::CommodityInfo> for Module<T, I>
{
